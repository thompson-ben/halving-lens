...
const STEPS: Step[] = [
  { id: "state-of-bitcoin", href: "/state-of-bitcoin", n: 1, title: "The State of Bitcoin", role: "What changed today, and why it matters." },
  { id: "accumulation", href: "/accumulation", n: 2, title: "Accumulation Index", role: "How attractive today's conditions look versus history." },
  { id: "historical-price-paths", href: "/historical-price-paths", n: 3, title: "Historical Price Paths", role: "How far previous cycles travelled from here." },
];
...
export function FlagshipJourney({ current }: { current: StepId }) {
  ...
  return (
    <section aria-label="The daily read" className="border-t border-white/[0.06] pt-8">
      ...
      <Link
        href={next.href}
        className="mt-4 group flex items-center justify-between gap-4 rounded-xl border border-white/[0.06] bg-white/[0.015] hover:bg-white/[0.03] hover:border-accent/30 px-5 py-4 transition-colors"
      >
        <div className="min-w-0">
          <div className="text-[10.5px] uppercase tracking-[0.16em] text-ink-500">Next in the daily read</div>
          <div className="mt-1 text-[15px] font-display text-ink-50 truncate">{next.title}</div>
          <div className="text-[12px] text-ink-400 mt-0.5">{next.role}</div>
        </div>
