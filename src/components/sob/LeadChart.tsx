...
export function LeadChart({ pick }: { pick: ChartOfWeekPick }) {
  return (
    <div className="card-glow p-4 sm:p-7 relative overflow-hidden">
      <div className="eyebrow text-accent mb-2">The week&rsquo;s lead chart</div>
      <h3 className="font-display text-headline font-medium tracking-tight-2 text-ink-50 leading-snug max-w-3xl">
        {pick.title}
      </h3>
      ...
      <div className="mt-5 grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
        <div>
          <div className="eyebrow text-ink-500 mb-1">What it shows</div>
          <p className="text-body text-ink-200 leading-relaxed">{pick.why[0] ?? pick.context}</p>
        </div>
        {(pick.why[1] || pick.context) && (
          <div>
            <div className="eyebrow text-ink-500 mb-1">Why it matters</div>
            <p className="text-body text-ink-200 leading-relaxed">{pick.why[1] ?? pick.context}</p>
          </div>
        )}
      </div>

      <p className="mt-5 pt-4 border-t border-white/[0.06] text-body text-ink-100 leading-relaxed max-w-measure">
        {pick.takeaway}
      </p>
      <div className="mt-3 flex items-center justify-between gap-3 flex-wrap">
        <span className="text-caption text-ink-600">Historical context, not a forecast.</span>
        <Link href={pick.link} className="text-caption text-accent hover:text-accent-soft">Open the live chart →</Link>
      </div>
    </div>
  );
}
