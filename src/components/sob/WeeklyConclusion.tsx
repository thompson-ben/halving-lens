...
export function WeeklyConclusion({
  ...
  return (
    <div className="card-glow p-5 sm:p-7 relative overflow-hidden">
      <div className="eyebrow text-accent mb-3">The week in one line</div>
      <p className="font-display text-headline text-ink-50 leading-snug tracking-tight-2 max-w-3xl">
        {verdict}
      </p>

      {presenter ? (
        <p className="mt-5 text-body text-accent leading-relaxed">
          {nextWatchTitle
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          ...
          <Link
            href="#subscribe"
            className="inline-flex items-center gap-2 h-11 px-5 rounded-lg bg-accent text-ink-950 text-body font-medium hover:bg-accent-soft transition-colors"
          >
            Get the Daily Bitcoin Cycle Brief
          </Link>
          <span className="text-caption text-ink-500">One clear read each morning · no hype, no predictions.</span>
        </div>
      )}
      <p className="mt-5 text-caption text-ink-600">Historical context. Not prediction. Not financial advice.</p>
    </div>
  );
}
