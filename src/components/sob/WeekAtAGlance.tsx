      ...
      <ol className="rounded-xl border border-white/[0.07] overflow-hidden divide-y divide-white/[0.05]">
        {rows.map((r) => (
          <li key={r.question}>
            ...
            >
              <span className="text-[11px] uppercase tracking-[0.14em] text-ink-500 sm:w-52 shrink-0 flex items-baseline gap-2">
                <span className="font-mono text-[9.5px] text-ink-600">{r.act}</span>
                {r.label}
              </span>
              <span className="text-[13px] text-ink-200 leading-relaxed group-hover:text-ink-50 transition-colors">
                {r.answer}
              </span>
            </button>
          </li>
...
export function ActBridge({ text }: { text: string }) {
  return (
    <p className="mt-8 mb-2 pl-4 border-l text-[13px] text-ink-400 leading-relaxed max-w-2xl" style={{ borderColor: "rgba(217,185,106,0.3)" }}>
      {text}
    </p>
  );
}
