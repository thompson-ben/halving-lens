    ...
    <div className="space-y-4">
      {!cmp.available ? (
        <div className="card p-6">
          <p className="text-body text-ink-300 leading-relaxed max-w-measure">
            ...
            {cmp.rows.map((r) => (
              <div key={r.key} className="card p-4">
                <div className="flex items-center justify-between gap-2">
                  <span className="eyebrow text-ink-500">{r.label}</span>
                  {r.delta && <span className={`text-caption tabular-nums ${TONE[r.tone]}`}>{r.delta}</span>}
                </div>
                <div className="mt-2 flex items-center gap-2 text-body text-ink-100">
                ...
                <div className={`mt-1.5 text-caption ${TONE[r.tone]}`}>{r.meaning}</div>
              </div>
            ))}
          </div>
        </>
      ...
      <div className="card-glow p-5 sm:p-6 relative overflow-hidden">
        ...
        <div className="flex items-center gap-2 text-accent mb-2">
          ...
          <span className="eyebrow">Last week we were watching</span>
          {watch.available && (
            <span className={`ml-auto inline-flex items-center rounded-full border px-2.5 py-0.5 text-micro ${OUTCOME_STYLE[watch.outcome].cls}`}>
              {OUTCOME_STYLE[watch.outcome].label}
            </span>
          )}
        </div>
        {watch.signal && (
          <h3 className="font-display text-headline font-medium tracking-tight-2 text-ink-50 leading-snug">
            {watch.signal}
          </h3>
        )}
        <p className="mt-2 text-body text-ink-300 leading-relaxed max-w-measure">{watch.detail}</p>
      </div>
    </div>
  );
}
