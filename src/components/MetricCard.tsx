...
const ZONE_TONE: Record<string, string> = {
  ...
  accumulation: "text-signal-green border-signal-green/30 bg-signal-green/10",
  neutral: "text-ink-300 border-white/10 bg-white/[0.03]",
  bullish: "text-signal-green border-signal-green/30 bg-signal-green/10",
...
export function MetricCard({ metric }: { metric: MetricDef }) {
  ...
  return (
    <Link
      ...
      className="card p-5 block group hover:border-accent/30 transition-colors"
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <h3 className="mt-1 font-display text-[16px] font-semibold text-ink-100 group-hover:text-accent transition-colors">
            {metric.name}
          </h3>
        </div>
        <span
          className={cn(
            "px-2 py-0.5 rounded-full border text-[10.5px] font-medium tracking-wide whitespace-nowrap",
            ZONE_TONE[zone],
          )}
        >
          {label}
      ...
      <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between text-[11px]">
        <span className="text-ink-400">Paid at <span className="text-ink-200">{metric.paidAt}</span></span>
