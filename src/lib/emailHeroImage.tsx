...
// The rotating "wow" hero visual for the daily/weekly email — a signature
// HalvingLens chart, selected by the day's strongest narrative (Similar Moments →
// Position → Drawdowns → Fear & Greed → Cycle context). Magazine-cover scale,
// dark+gold. Shared by the /email/chart routes so the render lives in one place.
//
// These routes are prerendered static (see the route files) exactly like the
// research-finding card images, which render reliably behind email image proxies.
// A per-day PATH segment (not a ?query) is used so Vercel's CDN serves the
// prebuilt static asset — a query-string variant of a static route was served
// unreliably and showed a broken image in the email.
...
async function renderChart(): Promise<ImageResponse> {
...
export async function emailHeroImage(): Promise<ImageResponse> {
  try {
...
// Date slugs to prerender for the dated route: today + a two-week window, so the
// send-day's image is always a prebuilt static asset on the live deployment
// (which is the previous day's build when the morning email goes out).
export function heroDateSlugs(days = 15): { d: string }[] {
  const out: { d: string }[] = [];
  const now = new Date();
  for (let i = 0; i < days; i++) {
    const dt = new Date(now.getTime() + i * 86_400_000);
    out.push({ d: dt.toISOString().slice(0, 10) });
  }
  return out;
}
