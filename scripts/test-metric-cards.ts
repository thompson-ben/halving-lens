...
console.log("Selection/export discipline (MW2-C):");
{
  ...
  check("one card → PNG, several → ordered ZIP via the shared makeZip", /from "@\/lib\/zip"/.test(pk) && /halvinglens-metric-pack-/.test(pk) && /files\.length === 1/.test(pk));
  check("share sheet only where image files are shareable (canShare probe)", /canShare\(\{ files:/.test(pk) && /navigator\.share\(\{ files:/.test(pk));
